...
def extract_key_data(text):
    ...
    tasks = ["Bending", "Putting on shoes", "Sleeping", "Standing", "Going up or down a flight of stairs", 
             "Walking through a store", "Driving for an hour", "Preparing a meal", "Yard work", "Picking up items off the floor"]
    data["difficulty_ratings"] = {}
    for task in tasks:
        match = re.search(fr"{task}:\s*(\d+)", text)
...
def save_to_database(data, db_path="ocr_results.db"):
    ...
    cursor.execute('''
        INSERT INTO ocr_results (
            ...
            pain, numbness, tingling, burning, tightness, difficulty_ratings
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        ...
        data["pain_symptoms"]["burning"], data["pain_symptoms"]["tightness"], json.dumps(data["difficulty_ratings"])
    ))

    connection.commit()
    connection.close()

def query_database(condition, db_path="ocr_results.db"):
    """Query the database for records matching a condition."""
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()

    # Execute query
    query = f"SELECT * FROM ocr_results WHERE {condition}"
